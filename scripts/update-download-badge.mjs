...
function defaultWindow() {
  ...
  if (Number.isNaN(now.getTime())) {
    throw new Error(
      `Invalid DOWNLOAD_BADGE_TODAY: ${process.env.DOWNLOAD_BADGE_TODAY}`,
    );
  }
...
async function main() {
  ...
  if (checkOnly && current !== next) {
    throw new Error(
      `${file} is stale. Run node scripts/update-download-badge.mjs`,
    );
  }
  ...
  console.log(
    `${pkg}: ${result.downloads} downloads (${result.start}..${result.end})`,
  );
}
